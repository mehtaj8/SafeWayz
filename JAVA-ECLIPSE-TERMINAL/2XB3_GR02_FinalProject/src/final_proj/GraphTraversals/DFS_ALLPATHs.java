/** @file DFS_ALLPATHS.java
@author Aditya Sharma
@brief Executes DFS for all possible paths with a given size
...
public class DFS_ALLPATHs {
	...
	private static ArrayList<Vector<String>> allPaths; // Will contain ALL Paths
	...
	public DFS_ALLPATHs(int maximumSize) {
		this.pathCount = 0;
		this.maxSize = maximumSize;
		this.fileName = "WeightedData.json";
		this.hashtable = generateHashtable.hashifiy(fileName);
		this.singularPath = new Stack <String>();
		this.storePath = new Stack <String>();
		this.allPaths = new ArrayList<Vector<String>>();
		this.allPathWeights = new ArrayList<ArrayList<Integer>>();
	}
	
	
	public ArrayList<Vector<String>> getPaths(){
	...
	private boolean findPath(String startPoint, String endPoint) {
		if (startPoint == endPoint) { // If the last point is the end point, save it to all paths and print the path
			...
			Vector<String> vectorized_path = new Vector<String>(this.storePath); 
	...
	public void DFS(String starting, String ending) { // used to execute code. This will be executed from the app.
